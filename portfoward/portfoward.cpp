...
void load_redirs()
{
	...
	while (fgets(buf, 1024, f)) {
		int lport, dport;
		char dest[1024];
		if (sscanf_s(buf, "%i %s %i", &lport, dest, sizeof(dest), &dport) == 3) {
			WCHAR bufW[1024];
...
unsigned int dnslookup(const char *dest)
{
	...
	if (sscanf_s(dest, "%d.%d.%d.%d", &a, &b, &c, &d) == 4)
...
DWORD WINAPI accepter(LPVOID lpThreadParameter)
{
    ...
    while ((n = accept(info->s, (sockaddr*)&sin, &ss)) != -1) {
        ...
        if (connect(d, (sockaddr*)&sin, sizeof(sin)) != 0) {
            sprintf_s(buf, sizeof(buf), "received a connection but can't connect to %s:%i\n", info->dest_host, info->dport);
			SetWindowTextA(status, buf);
            closesocket(n);
        } else {
            sprintf_s(buf, sizeof(buf), "connection to %s:%i established\n", info->dest_host, info->dport);
			SetWindowTextA(status, buf);
...
DWORD WINAPI reader(LPVOID lpThreadParameter)
{
	...
	sprintf_s(buf, sizeof(buf), "connection to %s:%i closed\n", ci->dest_host, ci->dport);
	SetWindowTextA(status, buf);
